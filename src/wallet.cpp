        ...
        CTxDB txdb("r");
        {
            ...
            while (true)
            {
                ...
                if (nChange > 0)
                {
                    ...
                    // -- don't put change output between value and reference outputs
...
bool CWallet::CreateTransaction(CScript scriptPubKey, int64_t nValue, std::string& sNarr, CWalletTx& wtxNew, CReserveKey& reservekey, int64_t& nFeeRet, const CCoinControl* coinControl)
{
    ...
    //    reference output will be for preceding output
    
    int nChangePos;
    bool rv = CreateTransaction(vecSend, wtxNew, reservekey, nFeeRet, nChangePos, coinControl);
    
    // -- reference will be added to mapValue later in FindStealthTransactions From CommitTransaction
    return rv;
}
...
bool CWallet::CreateStealthTransaction(CScript scriptPubKey, int64_t nValue, std::vector<uint8_t>& P, std::vector<uint8_t>& narr, std::string& sNarr, CWalletTx& wtxNew, CReserveKey& reservekey, int64_t& nFeeRet, const CCoinControl* coinControl)
{
    ...
    // -- shuffle inputs, change output won't mix enough as it must be not fully random for plantext references
    ...
    if (rv && narr.size() > 0)
    {
        for (unsigned int k = 0; k < wtxNew.vout.size(); ++k)
        {
            ...
            {
                printf("CreateStealthTransaction(): Error creating reference key.");
                break;
            };
...
bool CWallet::SendStealthMoneyToDestination(CStealthAddress& sxAddress, int64_t nValue, std::string& sNarr, CWalletTx& wtxNew, std::string& sError, bool fAskFee)
{
    ...
    if (sNarr.length() > 0)
    {
        ...
        {
            sError = "Reference encryption failed.";
            return false;
        };
        
        if (vchNarr.size() > 48)
        {
            sError = "Encrypted reference is too long.";
            return false;
        };
    };
            ...
            || vchEphemPK.size() != 33)
        {
                ...
                && vchEphemPK[1] == 'p')
            {
                    ...
                    && vchENarr.size() > 0)
                {
                    ...
                    snprintf(cbuf, sizeof(cbuf), "n_%d", nOutputIdOuter-1); // plaintext reference always matches preceding value output
                    mapNarr[cbuf] = sNarr;
                } else
                {
                    printf("Warning: FindStealthTransactions() tx: %s, Could not extract plaintext reference.\n", tx.GetHash().GetHex().c_str());
                };
            }
        ...
        BOOST_FOREACH(const CTxOut& txoutB, tx.vout)
        {
            ...
            for (it = stealthAddresses.begin(); it != stealthAddresses.end(); ++it)
            {
                    ...
                    && vchENarr.size() > 0)
                {
                    ...
                    {
                        printf("Decrypt reference failed.\n");
                        continue;
                    };
...
string CWallet::SendMoneyToDestination(const CTxDestination& address, int64_t nValue, std::string& sNarr, CWalletTx& wtxNew, bool fAskFee)
{
    ...
    if (sNarr.length() > 24)
        return _("Reference must be 24 characters or less.");
